...
namespace Owl.Domain
{
    ...
    public interface IRestfulApi
    {
        ...
        void Save(IEnumerable<AggRoot> adds, IEnumerable<AggRoot> updates, IEnumerable<AggRoot> removes);
        ...
        Response HandleMsg(string msgjson);

        /// <summary>
        /// 获取翻译
        /// </summary>
        string GetTranslation(string key, string content);

        /// <summary>
        /// 获取附件
        /// </summary>
        /// <param name="attidList"></param>
        /// <returns></returns>
        string GetFileForBase64(List<Guid?> attidList);

        /// <summary>
        /// 获取发票扫描信息
        /// </summary>
        /// <param name="FileName"></param>
        /// <param name="BufferBase64"></param>
        /// <returns></returns>
        string GetInvoiceScan(string FileName,string BufferBase64);
        ...
        /// <returns></returns>
        List<string> GetPermission(string msgjson);
    }
    /// <summary>
    /// restful api 代理
    /// </summary>
    public class RestProxy : RealProxy
    {
        ...
        public override IMessage Invoke(IMessage msg)
        {
            ...
            if (string.IsNullOrEmpty(Token) && methodcall.MethodName != "Token")
                throw new AlertException("error.owl.domain.restfulapi.notoken", "你尚未获得登录令牌，请获取令牌后再执行此操作");
            ...
            req.Headers.Add("cookie", $"owl_login_language={Owl.Feature.Cache.Session<string>("restapi_language", false)}"); //cookie中添加语言
            ...
            using (var response = req.GetResponse())
            {
                using (var reader = new System.IO.StreamReader(response.GetResponseStream()))
                {
                    if (returntype == typeof(void))
                        res = reader.ReadToEnd().DeJson<Response>();
                    else
        ...
        public static IRestfulApi Instance
        {
            get
            {
                return Owl.Feature.Cache.Thread<IRestfulApi>("owl.domain.restproxy.instance", () =>
                {
                    var Config = AppConfig.Section.GetConfig<Owl.Domain.Driver.Repository.RestConfigElement>();
                    return Create(Config.Url, Config.Mandt, Config.Login, Config.Password, false);
                });
            }
            set
            {
                Owl.Feature.Cache.Thread("owl.domain.restproxy.instance", value);
            }
        }
