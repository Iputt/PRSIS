    ...
    public static class Maintenance
    {
        ...
        static string Backup(DateTime time, IEnumerable<DomainModel> metas)
        {
            ...
            foreach (var meta in metas.OrderBy(s => s.Name))
            {
                ...
                MsgContext.Current.AppendTrackEntire("", "{0}:备份 {1} {2}\r\n", start, meta.Name, status);
            }
            MsgContext.Current.AppendTrackEntire("", "{0}:备份结束\r\n", DateTime.Now);
            return version;
        }
        ...
        public static void Restore(DateTime date, string version, params string[] models)
        {
            ...
            foreach (var meta in metas.OrderBy(s => s.Name))
            {
                ...
                MsgContext.Current.AppendTrackEntire("", "{0}:恢复 {1} {2}\r\n", start, meta.Name, status);
            }
            MsgContext.Current.AppendTrackEntire("", "{0}:恢复完成\r\n", DateTime.Now);
        }
        /// <summary>
        /// 恢复最近一次备份
        /// </summary>
